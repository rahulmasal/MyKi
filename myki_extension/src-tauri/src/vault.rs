//! Vault management for Myki Desktop
...
pub fn create_vault(db_path: &Path, password: &str, auto_lock_minutes: i32) -> Result<(), String> {
    ...
    // Store config
    conn.execute(
        "INSERT INTO vault_config (salt, password_hash, auto_lock_minutes, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, ?5)",
        params![
            &master_key.salt(),
            &password_hash,
            auto_lock_minutes,
...
pub fn toggle_favorite(conn: &Connection, id: &str) -> Result<bool, String> {
    ...
    conn.execute(
        "UPDATE credentials SET favorite = ?1 WHERE id = ?2",
        params![new_value, id],
    )
    .map_err(|e| e.to_string())?;
...
pub fn get_vault_status(db_path: &Path) -> Result<VaultStatus, String> {
    ...
    let auto_lock: i32 = conn
        .query_row(
            "SELECT auto_lock_minutes FROM vault_config LIMIT 1",
            [],
            |row| row.get(0),
        )
        .unwrap_or(5);
